...
if __name__ == '__main__':
    ...
    model = SCINet(output_len = args.horizon, input_len= 6, input_dim = 6, hid_size = args.hidden_size, num_stacks = 1,
                num_levels = 1, concat_len = 0, groups = args.groups, kernel = args.kernel, dropout = args.dropout,
                 single_step_output_One = args.single_step_output_One, positionalE =  args.positionalEcoding, modified = True).cuda()
    x = torch.randn(32, 6, 6).cuda()
    y = model(x)
    print(y)

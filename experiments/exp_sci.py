...
from utils.load_sci import DataLoaderH
...
class Exp_SCINet():
    ...
    def _build_model(self):
        if self.args.group == 'p':
            self.input_dim = 8

        if self.args.group == 'pf':
            self.input_dim = 28

        if self.args.group == 'pr':
            self.input_dim = 10

        if self.args.group == 'pi':
            self.input_dim = 11
    ...
    def _get_data(self):
        self.args.data = f'../datasets/{self.args.group}_t{self.args.horizon}.csv'

        return DataLoaderH(self.args.data, 0.6, 0.2, self.args.horizon, self.args.window_size, self.args.tra_date,
                           self.args.val_date, self.args.tes_date, self.args.normalize)
    ...
    def train(self):
        ...
        if self.args.resume:
            self.model, lr, epoch_start = load_model(self.model, save_path, model_name=self.args.dataset_name,
        ...
        for epoch in range(epoch_start, self.args.epochs):
            ...
            for tx, ty in data.get_batches(X, Y, self.args.batch_size, True):
                ...
                if self.args.single_step:  # single step
                    ...
                    if self.args.normalize == 3:
                        ...
                        loss_f = self.criterion(forecast[:, -1] * scale_last + bias_last,
                                                ty_last * scale_last + bias_last)
                        if self.args.stacks == 2:
                            loss_m = self.criterion(res * scale + bias, ty * scale + bias) / res.shape[
                                1]  # average results
                ...
                if iter % 100 == 0:
                    if self.args.stacks == 1:
                        ...
                        print('iter:{:3d} | loss: {:.7f}, loss_final: {:.7f}, loss_mid: {:.7f}'.format(iter,
                                                                                                       loss.item() / (
                                                                                                               forecast.size(
                                                                                                                   0) * data.m),
                                                                                                       loss_f.item() / (
            ...
            if self.args.stacks == 1:
                ...
                val_loss, val_rae, val_corr, val_rse_mid, val_rae_mid, val_correlation_mid = self.validate(data,
                                                                                                           data.valid[
                                                                                                               0],
                                                                                                           data.valid[
                                                                                                               1])
                test_loss, test_rae, test_corr, test_rse_mid, test_rae_mid, test_correlation_mid = self.validate(data,
                                                                                                                 data.test[
                                                                                                                     0],
                                                                                                                 data.test[
                                                                                                                     1])
            ...
            print(
                ...
                ' test rse {:5.4f} | test rae {:5.4f} | test corr  {:5.4f}'.format(
                    epoch, (time.time() - epoch_start_time), total_loss / n_samples, val_loss, val_rae, val_corr,
                    test_loss, test_rae, test_corr), flush=True)

            if val_loss < best_val:
                save_model(epoch, lr, self.model, save_path, model_name=self.args.dataset_name,
                           horizon=self.args.horizon)
                print('--------------| Best Val loss |--------------')
                best_val = val_loss
    ...
    def validate(self, data, X, Y, evaluate=False):
        ...
        if evaluate:
            save_path = os.path.join(self.args.save_path, self.args.model_name)
            self.model = \
                load_model(self.model, save_path, model_name=self.args.dataset_name, horizon=self.args.horizon)[0]
        ...
        if self.args.train is False and self.args.evaluate is True:
            data.sd['score'] = forecast_Norm[:, -1, -1].cpu().data.numpy()
            if not os.path.exists(self.args.output_path):
                os.mkdir(self.args.output_path)
            if not os.path.exists(self.args.output_path + "/prediction"):
                os.mkdir(self.args.output_path + "/prediction")

            for key, val in data.sd.groupby("dt"):
                val.to_csv(f"{self.args.output_path}/prediction/{key}.csv", index=False)
                print(f"{self.args.output_path}/prediction/{key}.csv生成！")
        ...
        if not self.args.single_step:  # single step
            for i in range(forecast_Norm.shape[1]):  # get results of each step
                lossL2_F = self.evaluateL2(forecast_Norm[:, i, :] * Scale + bias,
                                           target_Norm[:, i, :] * Scale + bias).item()
                lossL1_F = self.evaluateL1(forecast_Norm[:, i, :] * Scale + bias,
                                           target_Norm[:, i, :] * Scale + bias).item()
                if self.args.stacks == 2:
                    lossL2_M = self.evaluateL2(Mid_Norm[:, i, :] * Scale + bias,
                                               target_Norm[:, i, :] * Scale + bias).item()
                    lossL1_M = self.evaluateL1(Mid_Norm[:, i, :] * Scale + bias,
                                               target_Norm[:, i, :] * Scale + bias).item()
        ...
        print(
            '|valid_final rse {:5.4f} | valid_final rae {:5.4f} | valid_final corr  {:5.4f} |valid_final corr  {:5.4f} '.format(
                rse, rae, correlation, total_loss), flush=True)
        if self.args.stacks == 2:
            print(
